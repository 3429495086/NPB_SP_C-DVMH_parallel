...
void initialize()
{
  ...
  #pragma dvm region
  {
    #pragma dvm parallel(3) private(k,j,i)
  ...
  #pragma dvm region
  {
    #pragma dvm parallel(3) private(k,j,i,zeta,eta,xi,ix,iz,iy,m,Pxi,Peta,Pzeta,Pface)
    for (k = 0; k <= grid_points[2]-1; k++) {
      for (j = 0; j <= grid_points[1]-1; j++) {
        for (i = 0; i <= grid_points[0]-1; i++) {
          zeta = (double)k * dnzm1;
  ...
  #pragma dvm region
  {
    #pragma dvm parallel(3) private(k,j,i,xi,eta,zeta,temp, m)
    for (k = 0; k <= grid_points[2]-1; k++) {
      for (j = 0; j <= grid_points[1]-1; j++) {
        for (i = 0; i <= grid_points[0]-1; i++) {
          if (i == 0 || i == grid_points[0]-1 ||
              j == 0 || j == grid_points[1]-1 ||
              k == 0 || k == grid_points[2]-1) {
            xi   = (double)i * dnxm1;
            eta  = (double)j * dnym1;
            zeta = (double)k * dnzm1;
