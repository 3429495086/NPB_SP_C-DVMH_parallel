...
void exact_rhs()
{
  ...
  #pragma dvm region
  {
    #pragma dvm parallel(3) private(k,j,i,m)
  ...
  #pragma dvm region
  {
    #pragma dvm parallel(2) private(k,j,i,m,zeta,eta,xi,dtemp,dtpp,im1,ip1,ue,buf,cuf,q)
  ...
  #pragma dvm region
  {
    #pragma dvm parallel(2) private(i,j,k,m,zeta,eta,xi,dtpp,dtemp,jm1,jp1,ue,buf,cuf,q)
    for (k = 1; k <= grid_points[2]-2; k++) {
      for (i = 1; i <= grid_points[0]-2; i++) {
        zeta = (double)k * dnzm1;
        xi = (double)i * dnxm1;
  ...
  #pragma dvm region
  {
    #pragma dvm parallel(2) private(i,j,k,m,zeta,eta,xi,dtpp,dtemp,km1,kp1,ue,buf,cuf,q)
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (i = 1; i <= grid_points[0]-2; i++) {
        eta = (double)j * dnym1;
        xi = (double)i * dnxm1;
  ...
  #pragma dvm region
  {
    #pragma dvm parallel(3) private(k,i,j,m)
    for (k = 1; k <= grid_points[2]-2; k++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (i = 1; i <= grid_points[0]-2; i++) {
          for (m = 0; m < 5; m++) {
